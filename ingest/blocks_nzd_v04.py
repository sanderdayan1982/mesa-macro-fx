...
def enrich_fiscal(block: dict, cfg: dict, tf: Dict[str, Series], cal: Dict[str, Series], ni: Series, ecp: Series, resid_daily: Series, sc_level: float) -> dict:
    ...
    for k, label, src in (("tender_settled", "Tenders settled (bills + bonds; − settlement cash)", "NZDM tenders, settlement by convention T+1 bills / T+3 bonds"),
                          ...
                          ("bond_redeemed_market", "Bond redemptions to market holders (+; register market of the month-end before maturity − RBNZ LSAP holding − NZDM repurchases of the line)", "NZDM bonds on issue, every month-end; RBNZ D3 LSAP purchases/sales and repurchases"),
                          ("coupons_market_paid", "Coupons to market holders (+; semi-annual, market-held nominal net of LSAP and repurchases at the coupon date)", "NZDM bonds on issue, every month-end; RBNZ D3"),
    ...
    D["net_issuance_private_5d"] = entry("net_issuance_private_5d", wk, "Net issuance to the private sector, 5 sessions (− = drain)", "daily", unit, cfg, "derived", status="fresh" if wk else "unavailable",
                                         equivalence_note="v0.4 v2: −tenders settled + bill maturities + market bond redemptions + market coupons (net of the RBNZ LSAP holding and NZDM repurchases by line) + repurchases settled; reconciled to D10 bond maturities (two-sided from the first month-end of the register on file)")
